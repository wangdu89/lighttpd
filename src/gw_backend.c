...
static int gw_establish_connection(request_st * const r, gw_host *host, gw_proc *proc, pid_t pid, int gw_fd, int debug) {
	...
	#if 1
	int ret;
	if(strncmp(proc->connection_name->ptr, "tcp:0.0.0.0", 11) == 0)
	{
		char serverIP[32];
		char *index;
		
		snprintf(serverIP, 31, "%s", r->http_host->ptr);
		
		if(index = strstr(serverIP, ":"))
		{
			*index = '\0';
		}
		...
		ret = connect(gw_fd, (struct sockaddr*)&serv_addr, proc->saddrlen);
	}
	else
	{
		ret = connect(gw_fd, proc->saddr, proc->saddrlen);
	}
