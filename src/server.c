...
static int server_main_setup (server * const srv, int argc, char **argv) {
	...
	if (config_read(srv, "/etc/lighttpd/config/lighttpd.conf")) {
		log_error(srv->errh, __FILE__, __LINE__, "Can  read config file error.");
		return -1;
	}
	buffer_copy_string(srv->srvconf.modules_dir, "/usr/lib/lighttpd");
...
int main (int argc, char **argv) {
    int rc;
	//add by walter
	printf("========lighttpd server start /usr/lib/cgi-bin/avui/version.cgi==========");
	system("/usr/lib/cgi-bin/avui/version.cgi");
	//end
  #ifdef HAVE_GETUID
